...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        updates = [(param_var, param_var - self. learning_rate*grad_tensors[i]) for i,param_var in enumerate(param_vars)]
        "*** YOUR CODE HERE ***"
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        for i in range(len(param_vars)):
            new_v = (self.momentum * vel_vars[i]) - (self.learning_rate*grad_tensors[i])
            updates.append((vel_vars[i], new_v))
            updates.append((param_vars[i], param_vars[i] + new_v))
        # util.raiseNotDefined()
        return updates
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        train_losses = []
        val_losses = []
        train_data_itr = MinibatchIndefinitelyGenerator(train_data, 1, self.shuffle)
        val_data_itr = MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        ...
        for iter_ in range(self.iterations):
            ...
            """
            new_input_train_data, new_target_train_data = train_data_itr.next()
            train_loss = session.run(loss_tensor, feed_dict={placeholders[0]:new_input_train_data,placeholders[1]: new_target_train_data} )
            
            session.run(update_ops, feed_dict={placeholders[0]:new_input_train_data,placeholders[1]: new_target_train_data})

            new_input_val_data, new_target_val_data = val_data_itr.next()
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]:new_input_val_data,placeholders[1]: new_target_val_data})
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            ...
            new_input_train_data, new_target_train_data = trainGenerator.next()
            train_loss = session.run(loss_tensor, feed_dict={placeholders[0]:new_input_train_data,placeholders[1]: new_target_train_data} )
            
            session.run(update_ops, feed_dict={placeholders[0]:new_input_train_data,placeholders[1]: new_target_train_data})

            new_input_val_data, new_target_val_data = valGenerator.next()
            val_loss = session.run(loss_tensor, feed_dict={placeholders[0]:new_input_val_data,placeholders[1]: new_target_val_data})
